...
const CleanerAISettings: React.FC = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [settingsRes, templatesRes, responsesRes, prefsRes] = await Promise.all([
        legacyFetch('/cleaner/ai/settings'),
        legacyFetch('/cleaner/ai/templates'),
        legacyFetch('/cleaner/ai/quick-responses'),
        legacyFetch('/cleaner/ai/preferences'),
      ]);
  ...
  const updateSetting = async (key: string, value: any, enabled: boolean) => {
    try {
      await legacyFetch(`/cleaner/ai/settings/${key}`, {
        method: 'PATCH',
  ...
  const updatePreferences = async (updates: Partial<AIPreferences>) => {
    setSaving(true);
    try {
      await legacyFetch('/cleaner/ai/preferences', {
        method: 'PATCH',
  ...
  const createTemplate = async (template: Partial<Template>) => {
    try {
      await legacyFetch('/cleaner/ai/templates', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const deleteTemplate = async (templateId: string) => {
    ...
    try {
      await legacyFetch(`/cleaner/ai/templates/${templateId}`, {
        method: 'DELETE',
      });
      fetchAllData();
    } catch (error) {
