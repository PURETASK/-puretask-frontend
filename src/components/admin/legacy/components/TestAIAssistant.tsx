...
export function TestAIAssistant() {
  ...
  const loadTestData = async () => {
    ...
    try {
      ...
      try {
        const settingsRes = await legacyFetch('/cleaner/ai/settings');
        if (settingsRes.ok) {
      ...
      try {
        const templatesRes = await legacyFetch('/cleaner/ai/templates');
        if (templatesRes.ok) {
      ...
      try {
        const quickRes = await legacyFetch('/cleaner/ai/quick-responses');
        if (quickRes.ok) {
