 ...
 */
router.get('/global', jwtAuthMiddleware, async (req, res) => {
  try {
    ...
    const cleaners = await db.query(
      `SELECT id, full_name, email, 'cleaner' as type
       ...
       LIMIT $2`,
      [`%${query}%`, Math.floor(limit as number / 4)]
    );
    ...
    if (req.user?.role === 'client' || req.user?.role === 'admin') {
      const bookings = await db.query(
        ...
        req.user?.role === 'client'
          ? [`%${query}%`, Math.floor(limit as number / 4), req.user.id]
          : [`%${query}%`, Math.floor(limit as number / 4)]
      );
    ...
    res.json({ results: results.slice(0, limit as number) });
  } catch (error: any) {
 ...
 */
router.get('/autocomplete', jwtAuthMiddleware, async (req, res) => {
  try {
    ...
    const cleaners = await db.query(
      `SELECT DISTINCT full_name as text, id, 'cleaner' as type
       ...
       LIMIT $2`,
      [`%${query}%`, Math.floor(limit as number / 2)]
    );
    ...
    res.json({ suggestions: suggestions.slice(0, limit as number) });
  } catch (error: any) {
